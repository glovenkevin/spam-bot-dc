...
func (c *CommandService) generateAmythestCircleImage() *model.CommandNameHandlerParam {
	const (
		imageUrl = "image-url"
	)
	return &model.CommandNameHandlerParam{
		Command: &discordgo.ApplicationCommand{
			Name:        "generate-circle",
			Description: "Generate Circled Image using Amythest API",
			Type:        discordgo.ChatApplicationCommand,
			Options: []*discordgo.ApplicationCommandOption{
				{
					Type:        discordgo.ApplicationCommandOptionString,
		...
		Handler: func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			...
			userId := i.Member.User.ID
			imgByte, err := c.amythestRepo.GenerateCircle(context.Background(), url)
			if err != nil {
				c.log.Error(err)
				s.ChannelMessage(i.ChannelID, fmt.Sprintf("Fail to generate image for <@%s>", userId))
				return
